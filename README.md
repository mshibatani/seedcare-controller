...
- ESP32で温度センサーとリレーを使い、設定温度の範囲でヒーターをリレーで100V電源をON/OFFする。
- 土壌水分センサーの値も同時にモニタリングし、すべてのデータをMQTTでRaspberry Piのサーバーに送信し記録する。(機能を利用するにはMQTTサーバーが必要です)
-- https://github.com/mshibatani/seedcare #サーバー側のモニタープロジェクト
- Webサーバーを内蔵しており、現在データの表示、設定の変更やファームウェアアップデートが行える。
